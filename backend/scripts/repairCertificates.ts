...
import { checkAndAwardCertificate, evaluateAdvancedCertifications  } from '../services/certificateService';
...
async function run() {
  ...
  try {
    ...
    const quizMap: any = {}; // lessonId -> quiz
    allQuizzes.forEach(q => quizMap[(q.lessonId as any).toString()] = q);
