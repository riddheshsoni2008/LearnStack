...
const getLeaderboard = async (req, res) => {
  try {
    ...
    const leaderboard = registrations.map((reg, idx) => ({
      rank: idx + 1,
      name: (reg.userId as any)?.name || 'Unknown',
...
const getRoundQuestions = async (req, res) => {
  try {
    ...
    if (round > 1) {
      ...
      if (!prevSubmission || (prevSubmission.status as any) === 'disqualified') {
        return res.status(403).json({
          success: false,
  ...
  try {
    ...
    const exportData = participants.map((p, idx) => {
      const userSubmissions = submissions.filter(
        s => (s.userId as any).toString() === (p.userId as any)?._id?.toString()
      );
