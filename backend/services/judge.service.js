 ...
 */
const evaluateSubmission = (projectFiles) => {
  // Initialize scores
  let authScore = 0;
  ...
  const findFile = (substring) => {
    return projectFiles.find(f => f.path.toLowerCase().includes(substring.toLowerCase()));
  };

  // Helper to count occurrences of a query
  const checkContent = (file, regex) => {
  ...
  for (let filePath in importsMap) {
    ...
    imports.forEach(imp => {
      ...
      }) || imp.includes('react') || imp.includes('framer') || imp.includes('axios') || imp.includes('express') || imp.includes('mongoose') || imp.includes('bcrypt') || imp.includes('jsonwebtoken');
