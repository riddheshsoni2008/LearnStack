...
const HACKATHON_DURATION_HOURS = parseInt(process.env.HACKATHON_DURATION_HOURS as string) || 72;
const HACKATHON_BREAK_MINUTES = parseInt(process.env.HACKATHON_BREAK_MINUTES as string) || 10;
...
const processCompletedHackathons = async (): Promise<void> => {
  const now = new Date();
  ...
  for (const hackathon of completedHackathons) {
    ...
    for (const reg of registrations) {
      ...
      await Certificate.findOneAndUpdate(
        { userId: reg.userId, certificateType: certType, hackathonId: hackathon._id } as any,
        {
          userId: reg.userId,
...
const transitionUpcomingHackathons = async (): Promise<void> => {
  const now = new Date();
...
const ensureHackathonExists = async (): Promise<void> => {
...
const autoCreateNextHackathon = async (): Promise<void> => {
  const now = new Date();
