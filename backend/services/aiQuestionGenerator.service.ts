...
const ROUND_PROMPTS: Record<number, { count: number; difficulty: string; instructions: string }> = {
  1: {
    count: 3,
    difficulty: 'easy',
 ...
 */
const generateQuestionsForRound = async (
  ...
  if (!forceRegenerate) {
    const Hackathon = require('../models/Hackathon').default;
    const hackathon = await Hackathon.findById(hackathonId);
    if (hackathon) {
      const round = hackathon.rounds.find((r: any) => r.roundNumber === roundNumber);
    ...
    if (hackathon) {
      ...
      if (round) {
        ...
        if (forceRegenerate) {
          ...
          const existingIds = round.questionIds.map((id: any) => id.toString());
 ...
 */
const generateAllRoundQuestions = async (
  hackathonId: mongoose.Types.ObjectId | string,
  createdBy: mongoose.Types.ObjectId | string,
  forceRegenerate: boolean = false
): Promise<{ round1: any[]; round2: any[]; round3: any[] }> => {
  const results: any = { round1: [] as any[], round2: [] as any[], round3: [] as any[] };
