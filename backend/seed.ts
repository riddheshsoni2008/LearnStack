...
import { getChallenge  } from './data/codingChallenges';
...
const seedData = async () => {
  try {
    ...
    for (const badge of BADGES) {
      await Badge.create(badge as any);
      const rarityColors: any = { common: '⚪', rare: '🔵', epic: '🟣', legendary: '🟡' };
    ...
    for (let trackInfo of courseData) {
      ...
      for (let lessonInfo of trackInfo.lessons) {
        ...
        const lesson = await Lesson.create({
          ...
          xpReward: 10 + (lessonInfo.weekNumber * 5 as any),
          order: orderCounter++,
