...
export default function HackathonDetailsPage() {
      ...
      try {
        ...
        if (user) {
          const statusRes = await fetch(
            `/api/hackathons/${params.slug}/my-status`,
            {
              credentials: "include",
            },
          );
  ...
  const cutoffTime = hackathon.endDate
    ? new Date(new Date(hackathon.endDate).getTime() - 12 * 60 * 60 * 1000)
    : null;
  ...
  let primaryCtaAction = () =>
    router.push(`/hackathon/${params.slug}/register`);
  let primaryColor = "from-[#6c5ce7] to-[#00cec9]";

  if (isRegistered) {
    if (
      isCompleted ||
      regData.status === "winner" ||
      regData.status === "runner_up"
    ) {
      ...
      regData.status === "qualified" ||
      regData.status === "ROUND_2_ACTIVE" ||
      regData.status === "ROUND_3_QUALIFIED" ||
      regData.status === "PASSED"
    ) {
      ...
      if (
        nextRoundConfig &&
        (nextRoundConfig.status === "active" || DEVELOPMENT_MODE)
      ) {
        ...
        primaryCtaAction = () =>
          router.push(`/hackathon/${params.slug}/round/${eligibleRound}`);
      ...
      if (
        ...
        if (eligibleRound === 1) {
          ...
          primaryCtaAction = () =>
            router.push(`/hackathon/${params.slug}/round/1`);
          primaryColor = "from-green-500 to-emerald-500";
        } else {
          const isRoundActive =
            DEVELOPMENT_MODE ||
          ...
          if (isRoundActive && (isActive || DEVELOPMENT_MODE)) {
            primaryState = `ROUND_${eligibleRound}_AVAILABLE`;
            primaryCtaTitle =
              eligibleRound === hackathon.rounds.length
                ? "🏆 Final Round Live"
                : "✅ Congratulations! You qualified for Round " +
                  eligibleRound;
            primaryCtaLabel =
              eligibleRound === hackathon.rounds.length
                ? "Start Final Round"
                : `Start Round ${eligibleRound}`;
            primaryCtaAction = () =>
              router.push(`/hackathon/${params.slug}/round/${eligibleRound}`);
            primaryColor = "from-green-500 to-emerald-500";
          } else {
    ...
    primaryCtaAction = () =>
      router.push(`/hackathon/${params.slug}/leaderboard`);
    primaryColor = "from-gray-600 to-gray-500";
  }
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        <div className="mb-8">
          <Link
            href="/hackathon"
            className="text-sm text-[var(--primary-light)] hover:underline mb-4 inline-block"
          >
            ← Back to Hackathons
          </Link>
          ...
          <div className="mb-8 glass rounded-2xl p-6 overflow-x-auto hide-scrollbar">
            ...
            <div className="flex items-center min-w-max px-4">
              ...
              {hackathon.rounds.map((round, idx) => {
                ...
                const isFail =
                  submission &&
                  ["disqualified", "DISQUALIFIED", "FAILED"].includes(
                    submission.status,
                  );
                  ...
                  idx === 0
                    ...
                    : prevSubmission &&
                      [
                        ...
                        "COMPLETED",
                        "AUTO_SUBMITTED",
                      ].includes(prevSubmission.status);
                const lineClass = prevDone
                  ? "bg-emerald-500"
                  : "bg-[var(--border)]";
                ...
                if (isDone) {
                  ...
                  nodeClass =
                    "bg-[var(--primary)]/20 border-[var(--primary)] text-[var(--primary-light)] animate-pulse";
                  icon = "🔥";
                }
                ...
                return (
                  <div key={round.roundNumber} className="flex items-center">
                    {/* Connecting Line */}
                    <div
                      className={`w-16 sm:w-24 h-1 ${lineClass} transition-colors`}
                    />

                    {/* Node */}
                    <div className="flex flex-col items-center relative">
                      <div
                        className={`w-10 h-10 rounded-full border-2 flex items-center justify-center text-sm font-bold z-10 transition-colors ${nodeClass}`}
                      >
                        {icon}
                      </div>
                      <span
                        className={`text-xs font-bold mt-2 absolute top-12 whitespace-nowrap ${isEligible && !isDone && !isFail ? "text-[var(--primary-light)]" : "text-[var(--text-muted)]"}`}
                      >
              ...
              <div className="flex items-center">
                <div
                  className={`w-16 sm:w-24 h-1 transition-colors ${regData.status === "winner" ? "bg-yellow-500" : "bg-[var(--border)]"}`}
                />
                <div className="flex flex-col items-center relative">
                  <div
                    className={`w-10 h-10 rounded-full border-2 flex items-center justify-center text-lg z-10 transition-colors ${
                      regData.status === "winner"
                        ? "bg-yellow-500/20 border-yellow-500 text-yellow-400"
                        : "bg-[var(--surface-light)] border-[var(--border)] grayscale opacity-50"
                    }`}
                  >
                    🏆
                  </div>
                  <span className="text-xs font-bold mt-2 absolute top-12">
                    Winner
                  </span>
                </div>
              </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            <div>
              ...
              <p className="text-white/80 text-lg">
                {isRegistered
                  ? `Current Score: ${regData.totalScore || 0} pts`
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {hackathon.rounds?.length > 0 && (
              <div>
                <h2 className="text-2xl font-black mb-6 flex items-center gap-3">
                  <span className="text-[var(--primary)]">📊</span> Hackathon
                  Rounds
                </h2>
                <div className="grid gap-4">
                  {hackathon.rounds.map((round) => {
                    ...
                    const isEligible =
                      isRegistered && regData.currentRound >= round.roundNumber;
                    ...
                    return (
                      <div
                        ...
                        className={`glass rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all ${
                          canAccess
                            ? "border-[var(--primary)] shadow-[0_0_15px_rgba(108,92,231,0.2)]"
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[var(--text-muted)] mt-1 flex flex-wrap gap-2">
                              ...
                              <span>•</span>
                              <span>
                                🎯 {round.qualifyingScore} pts to qualify
                              </span>
                              <span>•</span>
                              <span className="uppercase text-[var(--primary-light)] font-bold">
                                {round.difficulty}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="w-full sm:w-auto flex-shrink-0">
                          {canAccess ? (
                            <Link
                              href={`/hackathon/${params.slug}/round/${round.roundNumber}`}
                              className="btn-primary !py-2 w-full sm:w-auto text-center block"
                            >
                              {round.roundNumber === 1
                                ? "🚀 Start Round 1"
                            ...
                            <button
                              disabled
                              className="btn-secondary !py-2 w-full sm:w-auto opacity-50 cursor-not-allowed"
                            >
            ...
            {/* Prize Pool */}
            {hackathon.prizePool &&
              (hackathon.prizePool.first ||
                hackathon.prizePool.second ||
                hackathon.prizePool.third) && (
                <div className="pt-4">
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {hackathon.prizePool.first && (
                      <PrizeCard
                        rank={1}
                        prize={hackathon.prizePool.first}
                        delay={0}
                      />
                    )}
                    {hackathon.prizePool.second && (
                      <PrizeCard
                        rank={2}
                        prize={hackathon.prizePool.second}
                        delay={0.1}
                      />
                    )}
                    {hackathon.prizePool.third && (
                      <PrizeCard
                        rank={3}
                        prize={hackathon.prizePool.third}
                        delay={0.2}
                      />
                    )}
                  </div>
            ...
            <div className="glass rounded-3xl p-6 border border-[var(--border)] sticky top-24">
              ...
              <div className="space-y-4 text-sm">
                {[
                  {
                    label: "Participants",
                    value: `${hackathon.currentParticipants || 0}${hackathon.participantLimitMode === "custom" ? `/${hackathon.maxParticipants}` : ""}`,
                    icon: "👥",
                  },
                  {
                    label: "Total Rounds",
                  ...
                  },
                  {
                    label: "Registration",
                    value:
                      hackathon.registrationType === "free"
                        ? "Free"
                        : `₹${hackathon.entryFee}`,
                    icon: "💰",
                  },
                  {
                    label: "Mode",
                    value: hackathon.hackathonMode === "team" ? "Team" : "Solo",
                    icon: "🎯",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="flex items-center justify-between pb-3 border-b border-[var(--border)] last:border-0 last:pb-0"
                  >
