...
const BADGE_STYLES = {
  ...
  QUALIFIED: { bg: "bg-emerald-500/20", text: "text-emerald-400", border: "border-emerald-500/30", icon: "✅", label: "Qualified" },
  ROUND_2_ACTIVE: { bg: "bg-blue-500/20", text: "text-blue-400", border: "border-blue-500/30", icon: "🔥", label: "Round 2 Active" },
  PASSED: { bg: "bg-emerald-500/20", text: "text-emerald-400", border: "border-emerald-500/30", icon: "⭐", label: "Passed" },
  FAILED: { bg: "bg-red-500/20", text: "text-red-400", border: "border-red-500/30", icon: "❌", label: "Failed" },
  ROUND_3_QUALIFIED: { bg: "bg-yellow-500/20", text: "text-yellow-400", border: "border-yellow-500/30", icon: "🏆", label: "Round 3 Qualified" },
...
export default function QualificationBadge({ status, size = "md", showIcon = true }: QualificationBadgeProps) {
  const style = BADGE_STYLES[status as keyof typeof BADGE_STYLES] || BADGE_STYLES.registered;
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 font-bold uppercase tracking-wider rounded-full border ${style.bg} ${style.text} ${style.border} ${sizeClasses[size as keyof typeof sizeClasses]}`}>
